...
pub const DeviceSeam = struct {
    ...
    pub const VTable = struct {
        ...
        /// `serveOne` never touches it again either way.
        place: *const fn (ptr: *anyopaque, kind: u8, path: []const u8, handle: i32) void,
        /// Take the node at `path` back out.
        drop: *const fn (ptr: *anyopaque, path: []const u8) void,
    };

    pub fn place(self: DeviceSeam, kind: u8, path: []const u8, handle: i32) void {
        self.vtable.place(self.ptr, kind, path, handle);
    }

    pub fn drop(self: DeviceSeam, path: []const u8) void {
        self.vtable.drop(self.ptr, path);
    }
};

/// What one `serveOne` did, for the caller that drives the helper's loop.
pub const Outcome = enum {
    /// A `Place` was read and handed to the seam.
    placed,
    /// A `Drop` was read and handed to the seam.
    dropped,
...
pub fn serveOne(fd: i32, seam: DeviceSeam) Outcome {
    ...
    if (rc == @sizeOf(Place) and !truncated) {
        ...
        {
            if (received) |handle| {
                seam.place(place.kind, place.path[0..place.path_len], handle);
                return .placed;
            }
        }
        ...
        {
            seam.drop(drop.path[0..drop.path_len]);
            return .dropped;
        }
    }
...
const StubSeam = struct {
    ...
    fn placeFn(ptr: *anyopaque, kind: u8, path: []const u8, handle: i32) void {
    ...
    fn dropFn(ptr: *anyopaque, path: []const u8) void {
        const self: *StubSeam = @ptrCast(@alignCast(ptr));
        self.drops += 1;
