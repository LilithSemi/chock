...
pub const Listener = struct {
    ...
    /// Generous, because a refused connection is a finding this would miss.
    ...
    pub fn start(gpa: std.mem.Allocator, io: std.Io) !*Listener {
        ...
        // namespace, where `127.0.0.1` is a different stack, so a route out by
        // any path the host holds must arrive at this one socket.
...
pub fn survivors(
    ...
    while (it.next(io) catch null) |entry| {
        ...
        if (namesAny(cmdline, prefixes)) {
            // Arguments are separated by a zero byte.
